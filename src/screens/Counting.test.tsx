...
describe('Counting screen', () => {
  ...
  it('toggles a candidate to disapproved when clicked', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
    const aliceBtn = screen.getByText('Alice').closest('button')!;
    await user.click(aliceBtn);
    expect(aliceBtn).toHaveClass('bg-red-100');
  });
  ...
  it('shows error when clicking next with no candidates disapproved', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
  ...
  it('advances to next ballot when at least one candidate is disapproved', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
  ...
  it('restores previous ballot state when clicking Previous', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
    // count ballot 1: disapprove Alice
    await user.click(screen.getByText('Alice').closest('button')!);
    await user.click(screen.getByText('PHIẾU KẾ TIẾP ▶'));
    // go back
    await user.click(screen.getByText(/PHIẾU TRƯỚC/));
    // back to ballot 1, Alice should be disapproved again
    expect(screen.getByText('Phiếu bầu số 1')).toBeInTheDocument();
    expect(screen.getByText('Alice').closest('button')).toHaveClass('bg-red-100');
  });
  ...
  it('navigates to report after confirming finish with disapproved candidates', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
    await user.click(screen.getByText('Alice').closest('button')!);
    await user.click(screen.getByText('KẾT THÚC KIỂM PHIẾU'));
  ...
  it('navigates to report after confirming finish with no disapproved (unchecked ballot skipped)', async () => {
    const user = userEvent.setup();
    renderWithContext(<Counting navigate={navigate} />, baseState);
    // do NOT disapprove anyone
    await user.click(screen.getByText('KẾT THÚC KIỂM PHIẾU'));
    await user.click(screen.getByText('Kết thúc'));
    expect(navigate).toHaveBeenCalledWith('report');
  });
