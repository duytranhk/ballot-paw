...
export default function Counting({ navigate }: Props) {
  ...
  function validateDisapproved(): boolean {
    if (disapproved.length === 0) {
      const error = `Phiếu bầu số ${state.ballotCount + 1} không hợp lệ — vui lòng loại ít nhất 1 ứng viên.`;
      showError(error);

      // Track validation error
      trackError("no_eliminations", error, "counting");
      return false;
    }
    return true;
  }
  ...
  function handleFinishConfirm() {
    ...
    trackCounting.complete(
      state.ballotCount + (disapproved.length > 0 ? 1 : 0), // Include final ballot if counted
