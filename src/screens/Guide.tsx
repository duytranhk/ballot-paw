...
export default function Guide({ navigate }: Props) {
  const { trackScreenView, updateActivity } = useAnalytics();

  // Track screen view when component mounts
  useEffect(() => {
    trackScreenView(ScreenName.GUIDE);
  }, [trackScreenView]);
    ...
    <ScreenLayout>
      ...
      <div className="flex-1 overflow-y-auto space-y-1 pb-2">
        <Section title="1. Thiết lập danh sách ứng viên">
          <Step num={1}>
            Nhập tên từng ứng viên vào ô văn bản, sau đó nhấn <b>+ THÊM</b> hoặc phím <b>Enter</b>.
          </Step>
          <Step num={2}>Lặp lại cho đến khi tất cả ứng viên đều có trong danh sách.</Step>
          <Step num={3}>
            Để xoá một ứng viên nhập sai, nhấn biểu tượng 🗑️ bên phải tên ứng viên đó.
          </Step>
          <Step num={4}>
            Khi danh sách đã đầy đủ, nhấn <b>BẮT ĐẦU KIỂM PHIẾU</b>.
          </Step>
        ...
        <Section title="2. Kiểm từng phiếu bầu">
          <Step num={1}>
            Mỗi màn hình tương ứng với <b>một phiếu bầu</b>. Tiêu đề hiển thị số thứ tự phiếu hiện
            tại.
          </Step>
          <Step num={2}>
        ...
        <Section title="3. Sửa phiếu vừa kiểm">
          <Step num={1}>
            Nhấn <b>◀ PHIẾU TRƯỚC</b> để quay lại phiếu trước đó và chỉnh sửa nếu kiểm nhầm.
          </Step>
          <Step num={2}>
            Sau khi sửa, nhấn <b>PHIẾU KẾ TIẾP ▶</b> để xác nhận lại.
          </Step>
        ...
        <Section title="4. Kết thúc kiểm phiếu">
          <Step num={1}>
            Sau khi kiểm xong tất cả phiếu, nhấn <b>KẾT THÚC KIỂM PHIẾU</b>.
          </Step>
          <Step num={2}>
            Nếu phiếu đang hiển thị chưa được kiểm (chưa loại ai), ứng dụng sẽ hỏi xác nhận — phiếu
            đó sẽ <b>không được tính</b> vào kết quả.
          </Step>
        ...
        <Section title="5. Xem & chia sẻ kết quả">
          <Step num={1}>
            Màn hình kết quả hiển thị tổng số phiếu và số phiếu từng ứng viên nhận được.
          </Step>
          <Step num={2}>
            Nhấn <b>CHIA SẺ KẾT QUẢ</b> để sao chép hoặc gửi kết quả qua ứng dụng khác.
          </Step>
          <Step num={3}>
            Nhấn <b>PHIÊN KIỂM PHIẾU MỚI</b> để đặt lại và bắt đầu phiên mới (danh sách ứng viên
            được giữ nguyên).
          </Step>
        </Section>
        ...
        <Section title="6. Lịch sử kiểm phiếu">
          <Step num={1}>
            Từ màn hình thiết lập, nhấn <b>XEM LỊCH SỬ KIỂM PHIẾU</b> để xem các phiên đã lưu.
          </Step>
          ...
          <Note>
            Dữ liệu được lưu trên thiết bị. Xoá bộ nhớ trình duyệt sẽ mất toàn bộ lịch sử.
          </Note>
        </Section>
      </div>
